    ...
    <article className={`rounded-xl border p-5 ${riskStyle[clause.riskLevel] ?? riskStyle.low}`}>
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <div className="flex gap-2">
          <Badge variant="outline">{isArabic ? arabicClauseCategoryLabel(clause.category) : clause.category}</Badge>
          <Badge variant="outline">{isArabic ? arabicRiskLabel(clause.riskLevel) : clause.riskLevel}</Badge>
        </div>
      </div>
...
function DateCard({ item, language }: { item: ContractDate; language: string }) {
  ...
  return (
    <article className="rounded-xl border border-slate-200 p-5 dark:border-slate-800">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <h4 className="font-bold text-slate-900 dark:text-white">{item.description || (isArabic ? arabicDateTypeLabel(item.type) : item.type)}</h4>
...
export default function ContractReport() {
  ...
  const clarityScore = normalizeClarityScore(contract.clarityScore);
